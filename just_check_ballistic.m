...
%   record_io=0;
 ...
 %  vout_first=p_Max(ceil(rand*n_v1));
   ...
   %disp('!')
    ...
    if(max_nn~=td+fd+ht && count_nn(td, ht) <= 200*ffd),
...
for fd_ii = ffd*3/4-ffd/4+1 : ffd*3/4+ffd/4
plot_den(:,:) = plot_den(:,:) + copy_atm(:,:,fd_ii);
plot_den2(:,:) = plot_den2(:,:) + copy_atm3(:,:,fd_ii);
end
plot_den(:,:) = plot_den(:,:) ./ ffd*2;
plot_den2(:,:) = plot_den2(:,:) ./ 20;
...
for fd_ii = ffd/4-ffd/4+1 : ffd/4+ffd/4
plot_den(:,:) = plot_den(:,:) + copy_atm(:,:,fd_ii);
plot_den3(:,:) = plot_den3(:,:) + copy_atm3(:,:,fd_ii);
end
plot_den(:,:) = plot_den(:,:) ./ ffd*2;
plot_den3(:,:) = plot_den3(:,:) ./ 10;

plotT1=plot_den;
for ht=1:hd
    for td=1:ftd
        if(plotT1(ht,td)<=0),plotT11(ht,td)=0;
        else plotT11(ht,td)=log10(plotT1(ht,td));
        end        
    end
end
imagesc(plotT11);
...
fclose(fid);
%% Write the ttime
fid=fopen('ttime_6071_0322.dat','w');
...
plotT=zeros(ftd,ffd);
plotT(:,:)=copyT(1,:,:);
flyd_mat = linspace(pi/3,pi/3,totaln);
%figure
%sphere;
%hold on;
totaln = 1;
fi=pi/4;
start_test=Radii.*[-1/sqrt(2) 0 -1/sqrt(2)];
...
%   record_io=0;
 ...
 %  vout_first=p_Max(ceil(rand*n_v1));
   ...
   %disp('!')
    ...
    if(col_p < 1 - exp(-real_col_p)&& ht <= fix(hd/3) && count_num(td, fd, ht)~=0),
        col_p = rand;
        disp('*');
        del_s = 0;
        col_cand = randi(count_num(td, fd, ht), 1);% collision candidatefrom the last time atmospheric velocity distribution
        num_col = num_col + 1;
        %disp('Bong!');
        num_Bong = num_Bong + 1;
        vv2 = new_v_record(td,fd,ht,col_cand);
        thi2 = pi * rand - pi / 2;
        fi2 = 2 * pi * rand;
